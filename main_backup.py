...
class ui_launcher:
    ...
    def create_ui(self):
        ...
        self.ui.pushButton_32.clicked.connect(self.stop_camera)
        self.ui.pushButton_31.clicked.connect(self.start_camera_thread)
        self.new_frame.valueChanged.connect(self.update_frame)

        self.ui.horizontalSlider.valueChanged.connect(
            lambda: self.slider_value_update(slider=self.ui.horizontalSlider, label=self.ui.lbleklem_1))
        self.ui.horizontalSlider_2.valueChanged.connect(
            lambda: self.slider_value_update(slider=self.ui.horizontalSlider_2, label=self.ui.lbleklem_2))
        self.ui.horizontalSlider_3.valueChanged.connect(
            lambda: self.slider_value_update(slider=self.ui.horizontalSlider_3, label=self.ui.lbleklem_3))
        self.ui.horizontalSlider_4.valueChanged.connect(
            lambda: self.slider_value_update(slider=self.ui.horizontalSlider_4, label=self.ui.lbleklem_4))
        self.ui.horizontalSlider_5.valueChanged.connect(
            lambda: self.slider_value_update(slider=self.ui.horizontalSlider_5, label=self.ui.lbleklem_5))
        self.ui.horizontalSlider_6.valueChanged.connect(
            lambda: self.slider_value_update(slider=self.ui.horizontalSlider_6, label=self.ui.lbleklem_6))
...
from Ters_Kinematik import forward_kinematics
...
class ui_launcher:
    ...
    def update_plot(self):
        thetas = [slider.value() for slider in self.sliders]
    ...
    '''def update(val, ax=None, pos_text=None):
        thetas = [slider.value() for slider in sliders]
        points, T_k = forward_kinematics(thetas)

        ax.cla()
        ax.plot(points[:, 0], points[:, 1], points[:, 2], 'bo-', linewidth=2)
        ax.set_xlim(-500, 500)
        ax.set_ylim(-500, 500)
        ax.set_zlim(-100, 500)
        ax.set_xlabel('X')
        ...
        # Uç işlevcinin konumunu yazdırma
        pos_text.set_val(f"X={T_k[0, 3]:.2f}\nY={T_k[1, 3]:.2f}\nZ={T_k[2, 3]:.2f}")'''
    ...
    def slider_hareket(self):
        value1 = self.ui.slider_joint1.value()
        value2 = self.ui.slider_joint2.value()
        value3 = self.ui.slider_joint3.value()
        value4 = self.ui.slider_joint4.value()
        value5 = self.ui.slider_joint5.value()
        value6 = self.ui.slider_joint6.value()

        print(f"Slider 1 Değeri: {value1}")
        print(f"Slider 2 Değeri: {value2}")
        print(f"Slider 3 Değeri: {value3}")
        print(f"Slider 4 Değeri: {value4}")
        print(f"Slider 5 Değeri: {value5}")
        print(f"Slider Uç işlevci Değeri: {value6}")
    ...
    def on_button_click(self):
        try:
            ...
            x_degeri = int(self.ui.lineEdit_7.text())
            # Girilen metni konsola yazdırır
            print("X = ", x_degeri)
            y_degeri = int(self.ui.lineEdit_8.text())
            print("Y = ", y_degeri)
            z_degeri = int(self.ui.lineEdit_9.text())
            print("Z = ", z_degeri)
            calculate_ik(x_degeri, y_degeri, z_degeri)
